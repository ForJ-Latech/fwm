...
[Top Bar Menu Items]
	[File]
		Change World - Allows the dungeon master to re-select the current world they are editing. Each "world" is completely independent from any other, so this can be used by a Dungeon Master to keep track of multiple RPG campaigns.

	[Global Settings]
		These settings affect Fantasy World Manager for every instance on the same computer.

		Set Max Number of Tabs - Changing this affects how many Tabs (Gods, Regions, NPCs, etc) the Main Window can have open at one time.
		Edit Hotkeys - Opens a new window that allows the Dungeon Master to customize the keys used to control the application.
		Toggle Webservice - Allows the Dungeon Master to turn off or on the Webservice.

	[World Settings]
		These settings are unique to each world made in Fantasy World Manager.

		Open World Settings - Opens a new window with settings for the currently opened world.
		Edit World God Statblock - Allows the Dungeon Master to change the default statblock used for any new god.
		Edit World NPC Statblock -Allows the Dungeon Master to change the default statblock used for any new npc.
	...
	[Search]
		...
		"I Forgot" Search - Opens additional search options for finding objects that the Dungeon Master may not remember by name.
	...
	[Help]
		...
		Open Advanced Web Service - Allows Dungeon Master to open a web service with editable properties; allows Dungeon Master to have a Co-Dungeon Master that can also edit the world.
		View Readme - Opens this! Hi!
		View Web Service URL - Displays a window with the URL to connect to the web service.
